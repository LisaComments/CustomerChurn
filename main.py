...
def make_predictions(input_df, input_dict):
  probabilities = {
    #predict_proba predicts 2 classes: pred class and pred possibility
    #0 & 1 are positive to determine if customer churned
    'XGBoost': xgboost_model.predict_proba(input_df)[0][1],
    'Random Forest': random_forest_model.predict_proba(input_df)[0][1],
    'K-Nearest Neighbors': knn_model.predict_proba(input_df)[0][1],
  }
  avg_probability = np.mean(list(probabilities.values())) #gets avg of predictions altogether
...
#define a function to explain model's prediction
...
varlous offers.
  ...
  You noticed a customer named {surname} nas a {round(probability * 100, 1)}% probability of churning.
  
  Here us the customer's information:
  {input_dict}
...
if selected_customer_option: #checks of customer is selected 

  selected_customer_id = int(selected_customer_option.split(" - ") [0])
  #extract the selected customer id
  #after splitting, we get the first element at [0]

  print("Selected Customer ID", selected_customer_id)
  #now get the customer surname, select the second element of the split list

  selected_surname = selected_customer_option.split(" - ")[1]
  #gets the surname and gets the 2nd element 

  print("Surname", selected_surname)

  #filter to get all data associated with this customer
  #do this by selecting loc accessor from data frame 
  selected_customer = df.loc[df['CustomerId'] == selected_customer_id].iloc[0] 
  #.iloc[0] gets you the actual row in the dataframe
  #this filters the data frame to find the exact row that matches the customer 

  print("Selected Customer", selected_customer)

  #create 2 columns 
  col1, col2 = st.columns(2)
  #creates 2 columns and stores them in col1 and col2 variables
  #add UI elements to these columns
  with col1: 
...
#puts into a data frame and a dictionary
...
#call to the generate email function
